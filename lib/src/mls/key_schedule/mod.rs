...
use crate::mls::framing::{Content, FramedContent, MlsGroupId, ProtocolVersion, WireFormat};
...
impl GroupContext {
    pub(crate) fn extract_joiner_secret(
        ...
        let extract_size = crypto_provider.hpke(cipher_suite)?.kdf_extract_size();
    ...
    pub(crate) fn extract_epoch_secret(
        ...
        let zero = vec![0u8; kdf_extract_size.into()];
        ...
        let raw_group_context = self.serialize_detached()?;
        let extract_size = crypto_provider.hpke(self.cipher_suite)?.kdf_extract_size();
...
impl InterimTranscriptHashInput {
    ...
    pub fn hash(
        ...
        let mut buf = BytesMut::new();
        serialize_opaque_vec(&self.confirmation_tag, &mut buf)?;
...
pub fn extract_psk_secret(
    ...
    for i in 0..psk_ids.len() {
        ...
        let psk_label = PSKLabel {
            id: psk_ids[i].clone(),
            index: u16::try_from(i).expect("psk_ids length can't be > u16::MAX"),
            count: u16::try_from(psk_ids.len()).expect("psk_ids length can't be > u16::MAX"),
        };
