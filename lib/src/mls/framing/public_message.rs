use crate::mls::framing::{
    deserialize_opaque_vec, serialize_opaque_vec, AuthenticatedContent, AuthenticatedContentTBM,
    Buf, BufMut, Bytes, CipherSuite, CryptoProvider, Deserializer, Error, FramedContent,
    FramedContentAuthData, GroupContext, Result, Sender, Serializer, WireFormat,
};
...
impl PublicMessage {
    ...
    pub fn verify_membership_tag(
        ...
        if let Some(membership_tag) = &self.membership_tag {
            let Ok(raw_auth_content_tbm) = self.authenticated_content_tbm(ctx).serialize_detached()
            else {
