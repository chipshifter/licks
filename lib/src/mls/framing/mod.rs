...
use crate::mls::key_schedule::{ConfirmedTranscriptHashInput, GroupContext};
use crate::mls::secret_tree::RatchetSecret;
...
use super::key_schedule::InterimTranscriptHashInput;
...
#[derive(Default, Debug, Clone, Eq, PartialEq)]
pub struct MlsEncodedMessage {
    pub version: ProtocolVersion,
    pub wire_message: WireMessage,
}

impl Deserializer for MlsEncodedMessage {
    fn deserialize<B>(buf: &mut B) -> Result<Self>
    where
        Self: Sized,
        B: Buf,
...
impl Serializer for MlsEncodedMessage {
    fn serialize<B>(&self, buf: &mut B) -> Result<()>
    where
        Self: Sized,
        B: BufMut,
...
impl AuthenticatedContent {
    ...
    pub fn new(
        ...
        let mut auth_content = Self {
            ...
            auth: FramedContentAuthData::default(),
        };
    ...
    pub(crate) fn interim_transcript_hash_input(&self) -> InterimTranscriptHashInput {
        InterimTranscriptHashInput {
            confirmation_tag: self.auth.confirmation_tag.clone(),
        }
    }
...
impl SenderData {
    ...
    pub fn new(leaf_index: LeafIndex, generation: u32) -> Result<Self> {
        let reuse_guard = random_bytes::<4>();
        Ok(Self {
            leaf_index,
            generation,
            reuse_guard,
...
pub(crate) fn expand_sender_data_key(
    ...
    let nk = crypto_provider.hpke(cipher_suite)?.aead_key_size();
...
pub(crate) fn expand_sender_data_nonce(
    ...
    let nn = crypto_provider.hpke(cipher_suite)?.aead_nonce_size();
    let ciphertext_sample = sample_ciphertext(crypto_provider, cipher_suite, ciphertext)?;
    crypto_provider.expand_with_label(
        cipher_suite,
        sender_data_secret,
