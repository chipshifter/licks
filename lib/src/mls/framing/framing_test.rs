...
use crate::mls::framing::{commit::*, group_info::*, proposal::*, *};
...
fn messages_test(tc: &MessagesTest) -> Result<()> {
    // Welcome
    let my_mls_welcome =
        MlsEncodedMessage::deserialize_exact(&tc.mls_welcome)?.serialize_detached()?;
    assert_eq!(&tc.mls_welcome, my_mls_welcome.as_ref());

    // (Verifiable)GroupInfo
    let my_mls_group_info =
        MlsEncodedMessage::deserialize_exact(&tc.mls_group_info)?.serialize_detached()?;
    assert_eq!(&tc.mls_group_info, my_mls_group_info.as_ref());

    // KeyPackage
    let my_key_package =
        MlsEncodedMessage::deserialize_exact(&tc.mls_key_package)?.serialize_detached()?;
    ...
    let my_public_message_application =
        MlsEncodedMessage::deserialize_exact(&tc.public_message_application)?
            .serialize_detached()?;
    assert_eq!(
    ...
    let my_public_message_proposal =
        MlsEncodedMessage::deserialize_exact(&tc.public_message_proposal)?.serialize_detached()?;
    assert_eq!(
    ...
    let my_public_message_commit =
        MlsEncodedMessage::deserialize_exact(&tc.public_message_commit)?.serialize_detached()?;
